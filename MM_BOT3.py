...
class Trading(Link):
    ...
    def update_signal(self):
        ...
        if not any(np.isnan(closes)):
            ...
            logger.info('unable to compute signal as closes not initialized')
    ...
    def calculate_order_sizes(self):
        # 계산된 매수와 매도 주문 크기 초기화
        bsize = 0
        asize = 0

        # 현재 포지션 리스크에 따라 매수 또는 매도 주문 크기 결정
        ...
        # 로그 기록
        logger.info(f"Calculated sizes -> Bid size: {bsize}, Ask size: {asize}")
    ...
    def orders_intent(self):
        ...
        # 주문 크기 계산
        bsize, asize = self.calculate_order_sizes()
